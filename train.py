...
def train(config='conf/config.yaml', **kwargs):
    ...
    if rank == 0:
        input = torch.randn(1, 300, 80)
        flops, params = profile(model, (input,))
        logger.info('speaker_model size: {}'.format(num_params))
        logger.info('speaker_model prameters: {0:.2f} M'.format(num_params / 1e6))
        logger.info('speaker_model FLOPs: {0:.2f} G'.format(flops / 1e9))
    ...
    configs['scheduler_args']['scale_ratio'] = 1.0 * world_size * configs[
        'dataloader_args']['batch_size'] / 128 #64
